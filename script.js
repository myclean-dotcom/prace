// script.js - Основная логика системы с вашими настройками
...
function initializeSystem() {
    // Заполняем города (только Новосибирск)
    renderCities();
    
    // Заполняем оборудование и химию
    renderEquipmentTags();
    renderChemistryTags();
    ...
    // Устанавливаем обработчик маски телефона
    setupPhoneMask();
    ...
    // Выбираем Новосибирск по умолчанию
    selectDefaultCity();
}

// ВЫБРАТЬ НОВОСИБИРСК ПО УМОЛЧАНИЮ
function selectDefaultCity() {
    const novosibirsk = CONFIG.CITIES.find(city => city.name === 'Новосибирск');
    if (novosibirsk) {
...
function selectCity(city) {
    // Снимаем выделение со всех городов
    document.querySelectorAll('.city-card').forEach(card => {
        card.classList.remove('selected');
    });
    
    // Находим и выделяем выбранный город
    ...
    showNotification(`Выбран город: ${city.name}`, 'info');
}

// ОТОБРАЖЕНИЕ ТЕГОВ ОБОРУДОВАНИЯ
...
function updateDifficultyDisplay() {
    ...
    // Выделяем соответствующую точку на шкале
...
function calculatePrice() {
    ...
    elements.orderTotal.value = calculatedPrice.toLocaleString('ru-RU') + ' ₽';
    
    // Обновляем зарплату мастера (если выбран процент)
...
function goToStep(step) {
    ...
    // Обновляем предпросмотр на шаге 3
    if (step === 3) {
        updatePreview();
    }
...
// ВАЛИДАЦИЯ ШАГА 1 (КАЛЬКУЛЯТОР)
function validateCalculatorStep() {
    if (!selectedCity) {
        showNotification('Пожалуйста, выберите город', 'error');
        return false;
    }
    
    if (!document.querySelector('.service-card.selected')) {
        showNotification('Пожалуйста, выберите тип уборки', 'error');
        return false;
    }
    
    const area = parseInt(elements.areaSize.value);
    if (!area || area < 10 || area > 1000) {
        showNotification('Пожалуйста, укажите корректную площадь (10-1000 м²)', 'error');
...
function validateDataStep() {
    ...
    if (!manager || manager.length < 2) {
        showNotification('Пожалуйста, укажите имя менеджера', 'error');
    ...
    if (!elements.customerStreet.value.trim()) {
        showNotification('Пожалуйста, укажите адрес уборки', 'error');
...
function updatePreview() {
    ...
    elements.previewCity.textContent = selectedCity ? selectedCity.name : 'Не выбран';
    elements.previewChannel.textContent = selectedCity ? selectedCity.channel : '—';
    
    // Тип уборки
    const selectedCard = document.querySelector('.service-card.selected');
    elements.previewType.textContent = selectedCard ? 
        selectedCard.getAttribute('data-type') : 'Не выбран';
    
    // Площадь
    elements.previewArea.textContent = `${elements.areaSize.value} м²`;
    ...
    if (date) {
        ...
        elements.previewDate.textContent = 'Не указана';
    }
}
...
function collectOrderData() {
    ...
    return {
        ...
        chemistry: selectedChemistry.join(', ') || '—',
...
// АЛЬТЕРНАТИВНЫЙ МЕТОД ОТПРАВКИ (GET)
async function sendOrderToServerAlternative(orderData) {
    try {
        // Преобразуем данные в параметры URL
...
function showSuccessResult(result) {
    // Обновляем данные в карточке результата
    ...
    // Показываем уведомление
    showNotification(`Заявка ${orderId} успешно создана!`, 'success');
}
...
// КОПИРОВАНИЕ ДАННЫХ ЗАЯВКИ
function copyOrderData() {
    const orderData = collectOrderData();
    
    // Форматируем текст для копирования
    let text = `🧹 ЗАЯВКА НА УБОРКУ №${orderData.order_id}\n`;
    text += `═══════════════════════════\n\n`;
    text += `📋 ОБЩАЯ ИНФОРМАЦИЯ:\n`;
    text += `Менеджер: ${orderData.manager}\n`;
    text += `Город: ${orderData.city}\n`;
    text += `Тип уборки: ${orderData.cleaning_type}\n`;
    text += `Площадь: ${orderData.area} м²\n`;
    text += `Стоимость: ${orderData.total_price.toLocaleString('ru-RU')} ₽\n`;
    text += `Дата: ${orderData.order_date} ${orderData.order_time}\n\n`;
    
    text += `👤 ДАННЫЕ КЛИЕНТА:\n`;
    text += `Имя: ${orderData.customer_name}\n`;
    text += `Телефон: ${orderData.customer_phone}\n`;
    text += `Адрес: ${orderData.customer_address}\n\n`;
    
    text += `🔗 Ссылка на заявку в Telegram: ${elements.resultChannelLink.href || '—'}\n`;
    text += `⏰ Создана: ${new Date().toLocaleString('ru-RU')}`;
    
    // Копируем в буфер обмена
    navigator.clipboard.writeText(text)
        .then(() => {
            showNotification('Данные заявки скопированы в буфер обмена!', 'success');
        })
        .catch(err => {
